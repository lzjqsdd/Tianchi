...
def LASSO(data,route,twn,time):
    ...
    #clf = xgb.XGBRegressor(max_depth=3,n_estimators=100,learning_rate=0.05)
    ...
    for day in range(7):
        for twn in y_tw_list:
            if pred_lasso_data[route][day][twn] > 0:
                ...
                pred_data[route][day][twn] = 0.5*pred_mean_data[route][day][twn] + 0.5*pred_lasso_data[route][day][twn] ##两种方法结果取均值
            else:
